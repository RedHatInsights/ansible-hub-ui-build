...
import { DataListItem, DataListItemRow, DataListItemCells, DataListCell, TextContent, Text, TextVariants, } from '@patternfly/react-core';
    ...
    CollectionListItem.prototype.render = function () {
        ...
        if (showNamespace) {
            cells.push(React.createElement(DataListCell, { isFilled: false, alignRight: false, key: 'ns' },
                React.createElement(Logo, { alt: company + ' logo', image: namespace.avatar_url, size: '50px' })));
        }
        ...
        cells.push(React.createElement(DataListCell, { key: 'content' },
            ...
            React.createElement("div", { className: 'entry pf-l-flex pf-m-wrap content' }, Object.keys(contentSummary.contents).map(function (k) { return (React.createElement("div", { key: k },
                React.createElement(NumericLabel, { label: k, number: contentSummary.contents[k] }))); })),
            React.createElement("div", { className: 'entry pf-l-flex pf-m-wrap' }, latest_version.metadata.tags.map(function (tag, index) { return (React.createElement(Tag, { key: index }, tag)); }))));
