...
var TableOfContents = /** @class */ (function (_super) {
    ...
    function TableOfContents(props) {
        ...
        _this.state = { collapsedCategories: [] };
        return _this;
    }
    TableOfContents.prototype.render = function () {
        var _this = this;
        var _a = this.props, className = _a.className, docs_blob = _a.docs_blob;
            ...
            React.createElement(Nav, { theme: 'light' },
                React.createElement(NavList, null, Object.keys(table).map(function (key) {
                    return table[key].length === 0
                        ? null
                        : _this.renderLinks(table[key], key);
                })))));
    };
    TableOfContents.prototype.parseLinks = function (docs_blob) {
        ...
        if (docs_blob.documentation_files) {
            for (var _i = 0, _b = docs_blob.documentation_files; _i < _b.length; _i++) {
                ...
                table.documentation.push({
                    display: this.capitalize(file.name
                        .split('.')[0]
                        .split('_')
                        .join(' ')),
    ...
    TableOfContents.prototype.renderLinks = function (links, title) {
        var _this = this;
        var isExpanded = !this.state.collapsedCategories.includes(title);
        return (React.createElement(NavExpandable, { key: title, title: capitalize(title + " (" + links.length + ")"), isExpanded: isExpanded, isActive: this.getSelectedCategory() === title }, links.map(function (link, index) { return (React.createElement(NavItem, { key: index, isActive: _this.isSelected(link) },
